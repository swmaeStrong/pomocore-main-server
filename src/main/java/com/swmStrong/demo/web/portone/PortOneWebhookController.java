...
public class PortOneWebhookController {
    ...
    public PortOneWebhookController(
            ...
            UserSubscriptionService userSubscriptionService, UserSubscriptionRepository userSubscriptionRepository) {
        ...
        try {
            ...
            switch (type){
                case "Transaction.Paid":
                    // 구독제 요금일 경우에는 연장 로직 수행
                    if (userSubscriptionRepository.existsByPaymentId(paymentId)) {
                        userSubscriptionService.extendUserSubscription(userId, paymentId);
                    }
            }
